...
export default function SigninModal({ isOpen, onClose }: SigninModalProps) {
  ...
  const isValid =
    validateLogin(loginVal) === "" && validatePassword(passwordVal) === "";
  ...
  return (
    ...
    >
      <div className={styles.authModalHeader}>
        <h2 className="montserrat-heavy">{t("header.sign")}</h2>
        <button
      ...
      <form className={styles.signinForm} noValidate onSubmit={handleSubmit}>
        <div
          className={`auth-field-group ${loginErr ? "invalid" : ""}`}
          id="auth-group-login"
        >
          <label className="montserrat-semi-bold" htmlFor="signin-login">
            {t("header.sign")}
          </label>
        ...
        <div
          className={`auth-field-group ${passwordErr ? "invalid" : ""}`}
          id="auth-group-password"
        >
          <label className="montserrat-semi-bold" htmlFor="signin-password">
            {t("modal.password")}
          </label>
          <div className={styles.authInputWrap}>
            <input
          ...
          {passwordErr && (
            <span className={styles.authFieldError}>{passwordErr}</span>
          )}
        </div>
